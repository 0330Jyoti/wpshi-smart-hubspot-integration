 (function($) { 
	...
	jQuery(document).on("change", "select[name=hubspot_module]", function(){
		
		jQuery('.loader').addClass('is-active');

		hubspot_module_name = jQuery(this).val();

		jQuery.ajax({
	        url: smart_hubspot_js.ajaxurl,
	        type: 'post',
	        data: {
	            'action':'hubspot_field',
	            'hubspot_module_name': hubspot_module_name
	        },
	        success: function( response ) {
	            jQuery("select[name=hubspot_field]").empty().append(response);
	            jQuery('.loader').removeClass('is-active');
	        },
	    });
	});
